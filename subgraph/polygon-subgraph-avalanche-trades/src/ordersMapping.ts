...
function _handleCancelOrder(account: Address, type: string, index: BigInt, timestamp: BigInt, transaction: string): void {
  let id = account.toHexString() + "-" + type + "-" + index.toString()
...
function _handleExecuteOrder(account: Address, type: string, index: BigInt, timestamp: BigInt, transaction: string): void {
  let id = account.toHexString() + "-" + type + "-" + index.toString()
  let order = Order.load(id)
...
export function handleCancelIncreaseOrder(event: CancelIncreaseOrder): void {
  _handleCancelOrder(event.params.account, "increase", event.params.orderIndex, event.block.timestamp, event.transaction.hash.toHexString());
  _storeStats("cancelledIncrease", "openIncrease")
}

export function handleExecuteIncreaseOrder(event: ExecuteIncreaseOrder): void {
  _handleExecuteOrder(event.params.account, "increase", event.params.orderIndex, event.block.timestamp, event.transaction.hash.toHexString());
...
export function handleCancelDecreaseOrder(event: CancelDecreaseOrder): void {
  _handleCancelOrder(event.params.account, "decrease", event.params.orderIndex, event.block.timestamp, event.transaction.hash.toHexString());
  _storeStats("cancelledDecrease", "openDecrease")
}

export function handleExecuteDecreaseOrder(event: ExecuteDecreaseOrder): void {
  _handleExecuteOrder(event.params.account, "decrease", event.params.orderIndex, event.block.timestamp, event.transaction.hash.toHexString());
...
export function handleCancelSwapOrder(event: CancelSwapOrder): void {
  _handleCancelOrder(event.params.account, "swap", event.params.orderIndex, event.block.timestamp, event.transaction.hash.toHexString());
  _storeStats("cancelledSwap", "openSwap")
}

export function handleExecuteSwapOrder(event: ExecuteSwapOrder): void {
  _handleExecuteOrder(event.params.account, "swap", event.params.orderIndex, event.block.timestamp, event.transaction.hash.toHexString());
